...
import Util
    ( Cell(..), CellTransform(Success, Fail), printBoard, assignCell )

startGame:: Int -> Int -> [Char] -> IO ()
startGame player modo symbols 
  | modo == 1 = playRound player 1 symbols board (5,5)      -- jogar modo 5x5
  | otherwise = playRound player 1 symbols board (7,7)        -- jogar modo 7x7
